...
export default function ServiceTemplatesManager({ open, onOpenChange, facility, course }) {
  const resolvedFacility = facility || course;
  const isEmbedded = typeof open === "undefined";
  ...
  const fetchData = useCallback(async () => {
    if (!resolvedFacility?.facility_id) return;

    setLoading(true);
    setError("");
    try {
      const [tpls, inv] = await Promise.all([
        api.serviceTemplates(resolvedFacility.facility_id),
        api.inventory(resolvedFacility.facility_id),
      ]);
      setTemplates(tpls);
      setInventory(inv);
    } catch (err) {
  ...
  useEffect(() => {
    if ((isEmbedded || open) && resolvedFacility?.facility_id) {
      fetchData();
    }
  }, [isEmbedded, open, resolvedFacility, fetchData]);
  ...
  const handleCreateTemplate = async (e) => {
    ...
    try {
      const created = await api.createServiceTemplate({
        facilityId: resolvedFacility.facility_id,
        name,
        description,
  ...
  const handleDeleteTemplate = async (id) => {
    ...
    try {
      await api.deleteServiceTemplate(id, resolvedFacility.facility_id);
